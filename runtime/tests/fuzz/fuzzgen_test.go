...
func TestFuzzgen(t *testing.T) {
	...
	fmt.Fprint(f, "import . \"github.com/onflow/cadence/runtime/parser2/lexer\"\n\n")

	var exampleTokenValues = map[string]map[string]bool{}
	...
	for testName, output := range testOutputs { //nolint:maprangecheck
		for _, line := range strings.Split(output, "\n") {
			if strings.Contains(line, "FUZZGEN:") {
				...
				if strings.HasPrefix(note, "TOKENDUMP\t") {
					a := strings.Split(note[len("TOKENDUMP\t"):], "\t")
					if tokenValue, err := strconv.Unquote(a[1]); err == nil {
						tokenType := a[0]
						if _, ok := exampleTokenValues[tokenType]; !ok {
							exampleTokenValues[tokenType] = map[string]bool{}
						}
						exampleTokenValues[tokenType][tokenValue] = true
					}
				}
			}
		}
	...
	fmt.Fprint(f, "//////////////////////////////////////////////\n")
	fmt.Fprint(f, "// example token values:\n\n")
	fmt.Fprint(f, "var exampleTokenValues = [TokenMax][]string {\n")
	for ty := lexer.TokenType(0); ty < lexer.TokenMax; ty += 1 {
		if values, ok := exampleTokenValues[ty.Name()]; ok {
			fmt.Fprint(f, "\t"+ty.Name()+": {\n")
			sorted := []string{}
			for v := range values {
				if len(v) > 100 || strings.HasPrefix(v, "\"\\u{") {
					continue
				}
...
func (p *progress) Write(bytes []byte) (n int, err error) {
	for _, b := range bytes {
		if b == '\n' {
			...
			p.wroteLines++
			if (p.wroteLines % 1000) == 0 {
				fmt.Fprint(os.Stderr, ".")
				if ((p.wroteLines / 1000) % 70) == 0 {
					fmt.Fprint(os.Stderr, "\nfuzzgen: ")
				}
			}
		}
